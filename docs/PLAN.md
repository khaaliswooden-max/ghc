...
| C+3a. STARK scaffold | parallel | `core/ghc-stark` ships a working prove/verify round trip. AIR uses a degree-7 Poseidon-like permutation over Goldilocks (2-element hash state + verdict column, 8-periodic round constants) with a quadratic extension for 95-bit security. All 5 tests pass (3 round-trip, 1 wrong-salt rejection, 1 commitment-binding). | **done (v0.2)** |
