...
//!
//! Width-3 (`rate = 2`, `capacity = 1`), `alpha = 17`, `8` full rounds,
//! `29` partial rounds. These constants are the documented `bls381-fr`
//! research parameters from arkworks `crypto-primitives`
//! `sponge::poseidon::tests::poseidon_parameters_for_test`. They are
//! suitable for v0.0.x research-grade use and are scheduled for
//! replacement with parameters from a future GHC trusted-setup
//! ceremony at v0.1 freeze (see `docs/PLAN.md`).
...
pub fn default_config<F: PrimeField>() -> PoseidonConfig<F> {
    let alpha = 17;
    let mds = vec![
        vec![
            F::from_str(
